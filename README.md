...
```mermaid
flowchart TD
    U([User text]) --> E[Extractor<br/>plain text → facts]
    E --> P[Planner<br/>check destination / month / budget]
    P -->|missing info| ASK([Ask the user]) --> ENDA([stop])
    P -->|all present| W[Weather + Best time<br/>OpenWeather + Serper]
    W --> D[Destination<br/>Serper: top places]
    D --> T[Transport<br/>Serper: reach + local]
    T --> B[Budget<br/>Serper research → Gemini estimate]
    B --> I[Itinerary / Composer<br/>Gemini: day-wise plan]
    I --> OUT([Day-wise itinerary + expenses])
```
